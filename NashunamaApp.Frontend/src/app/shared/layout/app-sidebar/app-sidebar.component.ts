...
import { MissingStockDateDto } from '@core/models/missing-stock.model';
...
export class AppSidebarComponent implements OnInit, OnDestroy {
  ...
  pendingDates: MissingStockDateDto[] = [];
  pendingTasksOpen = true;
  ...
  ngOnInit() {
    this.subscription.add(
      this.authService.missingStockNotification$.subscribe(notification => {
        this.pendingDates = notification?.missingDates ?? [];
  ...
  isActive(path: string): boolean {
    ...
    return currentUrl === normalizedPath || currentUrl.startsWith(`${normalizedPath}/`);
  }
  ...
  togglePendingTasks(): void {
    this.pendingTasksOpen = !this.pendingTasksOpen;
  }
  ...
  goToPendingDate(date: string): void {
    if (!date) return;
